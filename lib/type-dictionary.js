...
function GraphType(name, attributes) {
	...
	_.without(_.keys(this.attributes), 'type').forEach(function(key) {
		if (typedef.attributes.hasOwnProperty(key)) {
			// if the attribute type mapping is empty, then complain
			if (! typedef.attributes[key]) throw new Error(errors.INVALID_TYPE);
...
target object that relate to the definition of the type.  This is only relevant for
complex types (i.e. those types that define attributes)
*/
GraphType.prototype.createProperties = function(target) {
