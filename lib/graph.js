...
*/
Graph.prototype.create = function(nodeType, data) {
    // create the new graph node
    var CustomGraphNode = GraphNode,
        node,
        typeDef;

    // if the node type is an object, and not a String, then remap args
    if (typeof nodeType == 'object' && (! (nodeType instanceof String))) {
        data = nodeType;
        nodeType = undefined;
    }
    ...
    // create the new node
    node = new CustomGraphNode(this, nodeType, data);

    // if we have a type specified, then ensure the type has been defined
    if (node.type && (! this.types[node.type])) {
        throw new Error(errors.UNKNOWN_TYPE);
    }

    // get the type definition
    typeDef = node.type ? this.types[node.type] : null;
    ...
    this._queue('saveNode', node.data);
