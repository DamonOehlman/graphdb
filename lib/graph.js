...
function Graph(initOpts) {
    ...
    if (opts.connector) {
        debug('created graph, attempting to use the ' + opts.connector + ' connector');
    ...
    if (connector) {
        async.whilst(
            ...
            function(stepCb) {
                ...
                // add a callback to the operation args
                opArgs = nextOp.args.concat(function(err) {
