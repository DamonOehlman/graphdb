...
function Graph(initOpts) {
  ...
  // attach a remapId function if provided
  this.remapId = this.connector && this.connector.remapId;
...
Graph.prototype.find = function(searchParams, opts, callback) {
  ...
  return this.connector.find(this, searchParams, opts, function(err, results) {
    ...
    callback(null, results.map(function(data) {
      return new EntityType(graph, graph._parseData(data), targetType);
    }));
  });
};
...
Graph.prototype.save = function(callback) {
  ...
  debug('saving graph, operations queued = ' + this._ops.length);
...
Graph.prototype._parseData = function(data) {
  // create a clone of the data
  data = _.extend({}, data);

  // if we have a connector with an id remapper, then use it
  if (this.remapId) {
    data.id = this.remapId(data);
  }
